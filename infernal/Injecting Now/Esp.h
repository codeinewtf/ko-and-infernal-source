...
inline void TargetHotBar() {
	...
	if (Vars::Visuals::PlayerEsp::TargetHotBar)
	{
		...
		GUI::FillBoxRounded(Rect(Vars::Belt::Pos.x, Vars::Belt::Pos.y, 475, 75), { 21,21,21, 170 }, ScaleMode::StretchToFill);
				...
				if (ActWeapon && ActWeapon != nullptr && ActWeapon != 0)
				{
					...
					if (GetName && GetName != 0)
					{
						if (ActWeapon != nullptr)
						{
							...
							Color col = Target.Cur->GetActiveWeapon() == ActWeapon ? Color(0, 252, 242, 255) : Color(255, 255, 255, 0);
							...
							auto texture = Classes::get_texture(sprite); //(sprite)
							if (!texture)
								return;
