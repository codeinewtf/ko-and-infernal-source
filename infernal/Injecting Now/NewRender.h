...
namespace GUIElements
{
	...
	inline void CheckBox(const wchar_t* title, bool& value, float dist = 0, float textdist = 0) {
		...
		GUI::FillBoxRounded({ next_item_pos.x, next_item_pos.y, 12, 12 }, value ? Color(230, 50, 68, 255) : Color(60, 60, 60, 170), ScaleMode::StretchToFill, 2);
	...
	inline void Slider(const wchar_t* title, float& value, float min, float max, wchar_t* type = _(L"%d"))
	{
		...
		GUI::FillBoxRounded({ next_item_pos.x - 2, next_item_pos.y, ((float)(value * 150) / max), 16 }, Color(230, 50, 68, 255), ScaleMode::StretchToFill, 2);
...
void DrawNewMenu()
{
	...
	case 0:
		...
		GUIElements::TabChild(_(L""), { 25, 320 }, { 223, 270 });
		{
			...
			// auto desyncPercentage = (0.990f / 100) * Vars::Exploits::DesyncPercentage;
			...
			GUIElements::CheckBox(_(L"Auto Stop"), Vars::Exploits::AutoStopOnManipulation); // Vars::Exploits::ShowManipulation
		...
		{
			GUIElements::CheckBox(_(L"Pierce Materials"), Vars::Exploits::Pierce);
