...
// #include "bytes.h"
	void InitAssets()
	{
		//Unity::MyFont = Unity::LoadFromFile(_(L"C:\\Minoky\\lanerfont")); asset.assets
		//Unity::LogoPng = Unity::LoadFromFile(_(L"C:\\Windows\\Bundle\\minoku"));
		//Unity::NicotineBundle = Unity::LoadFromFile(_(L"C:\\Windows\\Bundle\\kozimozi"));
		//Unity::GGshader = Unity::LoadFromFile(_(L"C:\\Windows\\Bundle\\rust"));
		// Unity::VisCheckBundle = Unity::LoadFromFile(_(L"C:\\Nicotine\\Bundle\\vischeck.bundle"));
		// if (!Unity::MyFont) Unity::MyFont = Unity::LoadFromFile(_(L"C:\\Nicotine\\Bundle\\babekfont"));
		//Unity::ChamsMaterial = Unity::LoadFromFile(_(L"C:\\Minoky\\g.bundle"));
	}
...
//
	...
	inline void DrawBar(Vector2 start, Vector2 end, float a, float b, bool full = true)
	{
		if (a < 0) return;
		if (full)
			GUI::FillBoxRounded({ start.x, start.y , end.x, end.y + 1 }, Color(0, 0, 0, 170), ScaleMode::StretchToFill, 0);
		if ((a / b) > 1)
			a = b;
		GUI::FillBoxRounded({ start.x + 1, start.y + 1 , (end.x * (a / b)) - 1, end.y - 1 }, { 0,255,0,255 }, ScaleMode::StretchToFill, 0); // perc_col(a / b)
		StartGlRender(render_type::boxoutline, { start.x, start.y }, { end.x, end.y + 1 }, Color(21.f, 21.f, 21.f, 120.f));
	}
	...
	void InitializeAADrawing()
	{
		...
		if (!FindObjectFromInstanceID(AALineTextureInstanceID)) {
			...
			Create(AALineTexture, 1, 3, 1, 4, 0, 0);
		...
		if (!FindObjectFromInstanceID(LineTextureInstanceID)) {
			...
			// Create
			auto Create = reinterpret_cast<void(*)(uintptr_t, int, int, int, int, int, __int64)>((DWORD64)(g_Base + offsets::o_TextureCreate));
			Create(LineTexture, 1, 1, 1, 4, 0, 0);
	...
	void Hotkey(int* key, int* Type, bool& value, bool* GetKeyBind)
	{
		...
		if (*GetKeyBind)
		{
			...
			//FillBoxRounded({ next_item_pos.x + 226 , next_item_pos.y - 4, 14, 14 }, { 219, 65, 65, 255 }, ScaleMode::ScaleAndCrop, 0);
			String(textPos, _(L"Click Here!"), Color(255, 255, 255, 255), false, 8);
		...
		FillBoxRounded({ next_item_pos.x + 226 , next_item_pos.y - 2, 14, 14 }, { 219, 65, 65, 255 }, ScaleMode::ScaleAndCrop, 0); // 160,20,100,200
	...
	void TabChild(const wchar_t* title, const Vector2& pos, const Vector2& size)
	{
		...
		String({ window_pos.x + pos.x + 6.f, window_pos.y + pos.y - 20.f, 300, 40 }, title, { 255, 255, 255, 255 }, false, 11);
	...
	inline void CheckBox(const wchar_t* title, bool& value, float dist = 0, float textdist = 0) {
		...
		FillBoxRounded({ next_item_pos.x + 2, next_item_pos.y, 10, 10 }, !value ? Color(17, 17, 17, 255) : Color(219, 65, 65, 255), ScaleMode::ScaleAndCrop, 1); // 160,20,100,200
		//FillBoxRounded(Rect(!value ? next_item_pos.x + 208 - dist : next_item_pos.x + 222 - dist, next_item_pos.y - 2, 16, 16), !value ? Color(0, 182, 172, 255) : Color(0, 252, 242, 255), ScaleMode::ScaleAndCrop, 20);
		// 180, 40, 120, 255
		next_item_pos.y += 22;
	}
	...
	void Slider(const wchar_t* title, float& value, float min, float max, wchar_t* type = _(L"%s: %0.2f "))
	{
		if (GUI::open)
		{
			...
			StartGlRender(render_type::box, { next_item_pos.x, next_item_pos.y + 16 }, { ((float)(value * 240) / max), 6 }, Color(219, 65, 65, 255)); // 255, 80, 180, 255
	...
	int table = 0;
	void Tab(const wchar_t* name, int id, Vector2 tab_size = Vector2(78.f, 25.f)) {
		if (open)
		{
			if (is_mouse_in_box({ window_pos.x + (id * tab_size.x), window_pos.y }, { window_pos.x + (id * tab_size.x) + 78.f /*+ tab_size.x + 30*/, window_pos.y + tab_size.y}) && mouse_state)
				Vars::ActiveTab = id;
			...
			String({ window_pos.x + (id * tab_size.x) + 16.f, window_pos.y + (tab_size.y) - 22, 300, 40 }, (name), id == Vars::ActiveTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void Tab1(const wchar_t* name, int id, Vector2 tab_size = Vector2(78.f, 25.f)) {
		if (open)
		{
			if (is_mouse_in_box({ window_pos.x + (id * tab_size.x), window_pos.y }, { window_pos.x + (id * tab_size.x) + 78.f /*+ tab_size.x + 30*/, window_pos.y + tab_size.y }) && mouse_state)
				Vars::ActiveTab = id;
			...
			String({ window_pos.x + (id * tab_size.x) + 18.f, window_pos.y + (tab_size.y) - 22, 300, 40 }, (name), id == Vars::ActiveTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void Tab2(const wchar_t* name, int id, Vector2 tab_size = Vector2(78.f, 25.f)) {
		if (open)
		{
			if (is_mouse_in_box({ window_pos.x + (id * tab_size.x), window_pos.y }, { window_pos.x + (id * tab_size.x) + 78.f /*+ tab_size.x + 30*/, window_pos.y + tab_size.y }) && mouse_state)
				Vars::ActiveTab = id;
			...
			String({ window_pos.x + (id * tab_size.x) + 15.5f, window_pos.y + (tab_size.y) - 22, 300, 40 }, (name), id == Vars::ActiveTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void Tab3(const wchar_t* name, int id, Vector2 tab_size = Vector2(78.f, 25.f)) {
		if (open)
		{
			if (is_mouse_in_box({ window_pos.x + (id * tab_size.x), window_pos.y }, { window_pos.x + (id * tab_size.x) + 78.f /*+ tab_size.x + 30*/, window_pos.y + tab_size.y }) && mouse_state)
				Vars::ActiveTab = id;
			...
			String({ window_pos.x + (id * tab_size.x) + 24.f, window_pos.y + (tab_size.y) - 22, 300, 40 }, (name), id == Vars::ActiveTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void subTab(const wchar_t* name, int id, Vector2 tab_size = Vector2(78.f, 25.f)) {
		if (open)
		{
			if (is_mouse_in_box({ window_pos.x + (id * tab_size.x), window_pos.y + 25}, { window_pos.x + (id * tab_size.x) + 78.f /*+ tab_size.x + 30*/, window_pos.y + tab_size.y + 26 }) && mouse_state)
			...
			String({ window_pos.x + (id * tab_size.x) + 22.f, window_pos.y + 23 + (tab_size.y) - 20, 300, 40 }, (name), id == Vars::ActuveSubTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void DoubleSubTab1(const wchar_t* name, int id, Vector2 tab_size = Vector2(156.f, 25.f)) {
		if (open)
		{
			...
			String({ window_pos.x + (id * tab_size.x) + 52.5f, window_pos.y + 26 + (tab_size.y) - 23, 300, 40 }, (name), id == Vars::ActuveSubTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void DoubleSubTab2(const wchar_t* name, int id, Vector2 tab_size = Vector2(156.f, 25.f)) {
		if (open)
		{
			...
			String({ window_pos.x + (id * tab_size.x) + 49.f, window_pos.y + 26 + (tab_size.y) - 23, 300, 40 }, (name), id == Vars::ActuveSubTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void DoubleSubTab3(const wchar_t* name, int id, Vector2 tab_size = Vector2(156.f, 25.f)) {
		if (open)
		{
			...
			String({ window_pos.x + (id * tab_size.x) + 54.f, window_pos.y + 26 + (tab_size.y) - 23, 300, 40 }, (name), id == Vars::ActuveSubTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
	void DoubleSubTab4(const wchar_t* name, int id, Vector2 tab_size = Vector2(156.f, 25.f)) {
		if (open)
		{
			...
			String({ window_pos.x + (id * tab_size.x) + 56.f, window_pos.y + 26 + (tab_size.y) - 23, 300, 40 }, (name), id == Vars::ActuveSubTab ? Color{ 219, 65,65, 255 } : Color{ 105, 105,105, 255 }, false, 10);
		}
	}
